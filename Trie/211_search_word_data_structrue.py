...
#
# Logic:
# We use Trie as the data structure for the problem
...
class WordDictionary:
    ...
    def dfs(self, index: int, node: TrieNode) -> bool:
        if index >= len(self.word):
            return node.isWord
        if self.word[index] == ".":
            for trieNode in node.children.values():
                if self.dfs(index + 1, trieNode):
                    return True
            return False
        else:
            if self.word[index] not in node.children:
                return False
            return self.dfs(index + 1, node.children[self.word[index]])
