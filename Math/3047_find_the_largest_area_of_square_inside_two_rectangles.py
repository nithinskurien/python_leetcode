...
# Explanation:
#
# A square with side length 1 can fit inside the intersecting region of any two rectangles. Also, no larger square
# can, so the maximum area is 1. Note that the region can be formed by the intersection of more than 2 rectangles.
#
# Example 4:
#
#
...
#
# Logic: We can iterate over all the rectangles and check for intersections with the other rectangles. If we find an
# intersection we can calculate the area and keep track of the max area and return it.
        ...
        for i in range(n):
            for j in range(i + 1, n):
                if (bottomLeft[i][0] < topRight[j][0] and bottomLeft[j][0] < topRight[i][0]) and (
                    ...
                    res = max(res, min(top - bottom, right - left) ** 2)
        return res
