...
#
# Logic:
# We need to store the dict with the roman conversions. We can then check character by character if the next one is
# lesser than the current one if it is we can add the current number if it is not we need to subtract. We can then
# return the total.
...
class Solution:
    def romanToInt(self, s: str) -> int:
        ...
        for index in range(len(s) - 1):
            if keyMap[s[index]] < keyMap[s[index + 1]]:
                ans -= keyMap[s[index]]
            else:
                ans += keyMap[s[index]]
        ans += keyMap[s[-1]]
