...
# Example 1:
#
# Input: poured = 1, query_row = 1, query_glass = 1 Output: 0.00000 Explanation: We poured 1 cup of champange to the
# top glass of the tower (which is indexed as (0, 0)). There will be no excess liquid so all the glasses under the
# top glass will remain empty. Example 2:
#
# Input: poured = 2, query_row = 1, query_glass = 1 Output: 0.50000 Explanation: We poured 2 cups of champange to the
# top glass of the tower (which is indexed as (0, 0)). There is one cup of excess liquid. The glass indexed as (1,
# 0) and the glass indexed as (1, 1) will share the excess liquid equally, and each will get half cup of champange.
# Example 3:
#
...
#
# Logic:
# We can simulate the pouring and find the resulting volume in the glass.
    ...
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        for row in range(query_row):
            for col in range(row + 1):
                overflow = (glass[row][col] - 1) / 2
                if overflow > 0:
                    glass[row + 1][col] += overflow
                    glass[row + 1][col + 1] += overflow
