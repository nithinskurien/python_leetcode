...
#
# Logic:
# We can sort the array into ascending order and then find the difference between the first and k - 1 term of each k
# window elements. We can replace the result to keep the minimum value and return it after iterating the array.
...
class Solution:
    def minimumDifference(self, nums: list[int], k: int) -> int:
        ...
        for index in range(len(nums) - k + 1):
            diff = nums[index + k - 1] - nums[index]
            res = min(res, diff)
        return res